...
func (s *forced) Animate(fn string) {
	s.GI = initGI()
	for i := 0; i < s.N; i++ {
		s.addFrame(i + 1)
	}
...
func (s *forced) addFrame(count int) {
	palette := []color.Color{
		color.White,
		color.Black,
...
func (s *forced) clear(c *image.Paletted) {
...
func (s *forced) axes(c *image.Paletted) {
...
func (s *forced) addLabel(c *image.Paletted) {
	d := &font.Drawer{
		Dst:  c,
...
func (s *forced) drawSpring(c *image.Paletted, count int) {
	s.addCeil(c)
	s.addSpring(c, count)
	s.addCircle(c, count)
}

func (s *forced) addCeil(c *image.Paletted) {
	for i := s.GI.Pad; i < s.GI.SpWidth-s.GI.Pad; i++ {
		c.Set(i, s.GI.Ceil, c.Palette[1])
	}
}

func (s *forced) addSpring(c *image.Paletted, count int) {
	for i := 0; i < 100; i++ {
...
func (s *forced) springXY(i, count int) [2]int {
	n := s.GI.Coil[0]
	r := s.GI.Coil[1]
	d := s.GI.Coil[2]
...
func (s *forced) addCircle(c *image.Paletted, count int) {
	y := s.F(s.Xb[1] / float64(s.N) * float64(count))
	for i := 0; i < 100; i++ {
...
func (s *forced) circleXY(l, i int) [2]int {
	r := s.GI.CR
